...
def run_memory_size_benchmarks(
    ...
    df.to_csv(
        f"./results/{dataset_name}/{dataset_name}_memory_vs_size_{n_iterations}_iterations.csv",
        index=False,
    )

    return f"./results/{dataset_name}/{dataset_name}_memory_vs_size_{n_iterations}_iterations.csv"
...
def plot_mem_vs_size(save_dir: str, dataset_name: str):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f"./results/{dataset_name}/{dataset_name}_memory_vs_size.png")
    plt.close()
...
def main():
    ...
    parser.add_argument(
        "--dataset-idx", type=str, required=True, help="Dataset name"
    )
