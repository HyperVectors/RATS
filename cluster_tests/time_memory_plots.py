...
def main():
    ...
    output_dir = f"results/{dataset_index}/plots"

    os.makedirs(output_dir, exist_ok=True)

    # Load data
    time_csv = f"./results/{dataset_index}/{dataset_index}_time_benchmark.csv"
    mem_csv = f"./results/{dataset_index}/{dataset_index}_memory_benchmark.csv"
    ...
    plt.title(f"{dataset_index} Dataset Augmentation Time: RATSpy vs tsaug")
    plt.xticks(rotation=45, ha="right")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{dataset_index}_time_benchmark_bar.png")
    plt.close()
    ...
    plt.title(f"{dataset_index} Dataset Augmentation Peak Memory: RATSpy vs tsaug")
    plt.xticks(rotation=45, ha="right")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{dataset_index}_memory_benchmark_bar.png")
    plt.close()
    ...
    plt.ylabel("Peak Memory (MB)")
    plt.title(
        f"{dataset_index} Dataset Time vs Memory: RATSpy vs tsaug (per augmenter)"
    )
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{dataset_index}_time_vs_memory_scatter.png")
    plt.close()

    print(
        f"Plots saved as {output_dir}/{dataset_index}_time_benchmark_bar.png, "
        f"{output_dir}/{dataset_index}_memory_benchmark_bar.png, and "
        f"{output_dir}/{dataset_index}_time_vs_memory_scatter.png"
    )
