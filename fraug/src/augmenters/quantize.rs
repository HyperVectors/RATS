...
impl Augmenter for Quantize {
    fn augment_one(&self, x: &mut [f64]) {
        ...
        let level_set = (0..self.levels).map(|level| {min + level as f64 * step}).collect::<Vec<_>>();
        
        // Could be faster using e.g. binary search
        x.iter_mut().for_each(|v| {
            let i = level_set.iter().map(|&l| (l-*v).abs()).enumerate().fold((0, f64::INFINITY), |(i, a), (j, b)| if a > b { (j, b) } else { (i, a) }).0;
...
mod tests {
    ...
    #[test]
    fn quantize() {
        let mut series = vec![1.0; 11].iter().enumerate().map(|(i, _)| i as f64).collect::<Vec<_>>();

        let aug = Quantize::new(5);
        aug.augment_one(&mut series);

        assert_eq!(series, vec![0.0, 0.0, 2.0, 2.0, 4.0, 4.0, 6.0, 6.0, 8.0, 8.0, 8.0]);
    }
