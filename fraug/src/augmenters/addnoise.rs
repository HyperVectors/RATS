...
impl AddNoise {
    pub fn new(noise_type: NoiseType, bounds: Option<(f64,f64)>, mean: Option<f64>, std_dev: Option<f64>) -> Self {
        AddNoise { 
    ...
    fn augment_one(&self, x: &mut [f64]) {
        match self.noise_type {
            ...
            NoiseType::Slope => {
                ...
                x.iter_mut().enumerate().for_each(|(i, val)| *val += i as f64 * slope);
            },
        }
    }
}
...
mod tests {
    ...
    fn uniform() {
        ...
        series.iter().for_each(|&val| assert!(val >= 0.0 && val <= 2.0));
    ...
    fn spike() {
        ...
        let mut different = 0;
        series.iter().for_each(|&val| if val != 1.0 { different += 1; });
