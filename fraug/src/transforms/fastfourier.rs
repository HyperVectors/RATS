...
pub fn dataset_fft(dataset: &Dataset) -> Dataset {
    ...
    let mut freq_features = Vec::with_capacity(dataset.features.len());
...
pub fn dataset_ifft(dataset: &Dataset) -> Dataset {
    let mut planner = FftPlanner::new();
    let mut time_features = Vec::with_capacity(dataset.features.len());
    ...
    for sample in &dataset.features {
        ...
        let time_signal: Vec<f64> = buffer.iter().map(|c| c.re / len as f64).collect();
        time_features.push(time_signal);
    }
