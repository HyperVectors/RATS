...
plt.plot(df_aug.iloc[120, :], label="Augmented", alpha=0.5)
...
plt.fill_between(
    range(len(mean_orig)), mean_orig - std_orig, mean_orig + std_orig, alpha=0.2
)
plt.plot(mean_aug, label="Augmented Mean")
plt.fill_between(
    range(len(mean_aug)), mean_aug - std_aug, mean_aug + std_aug, alpha=0.2
)
